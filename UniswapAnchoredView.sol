...
contract UniswapAnchoredView is  UniswapConfig, Ownable {
    ...
    function addNewConfig(TokenConfig memory config) public onlyOwner {
        ...
        else if (config.priceSource == PriceSource.BACKEND) {
    ...
    function priceInternal(TokenConfig memory config) internal view returns (Error,uint) {
        if (config.priceSource == PriceSource.REPORTER || 
        config.priceSource == PriceSource.BACKEND) return (Error.NO_ERROR,prices[config.symbolHash].price);
    ...
    function validateAquaAndGammaPrice(address gToken,uint newPrice) external onlyOwner{
        
        TokenConfig memory config = getTokenConfigByGToken(gToken);
        prices[config.symbolHash].price = uint248(newPrice);
        emit PriceUpdated(config.symbolHash, newPrice);
